...
export default function WalletPage() {
  ...
  // Close the pending panel once the webhook marks the deposit as completed
  useEffect(() => {
    if (!depositInfo) return;
    const confirmed = transactions.some(
      (tx) => tx.type === 'deposit' && tx.status === 'completed' && tx.metadata?.reference === depositInfo.reference,
    );
    if (confirmed) {
      sileo.success({ title: 'Depósito confirmado', description: `${formatMXN(depositInfo.amount)} MXNe acreditados` });
      setDepositInfo(null);
      setCountdown(0);
    }
  }, [transactions, depositInfo]);
