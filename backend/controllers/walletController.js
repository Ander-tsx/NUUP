...
const handleVibrantWebhook = async (req, res) => {
  try {
    ...
    if (
      !vibrantService.verifyWebhookSignature(
        req.rawBody,
        signature,
        process.env.VIBRANT_WEBHOOK_SECRET,
      )
    ) {
    ...
    if (type === "deposit.confirmed") {
      ...
      if (!reference)
        return res.status(400).json({ error: "Missing deposit reference." });

      // 3. Atomically claim the pending transaction (pending → processing) so
      //    concurrent or duplicate deliveries can never credit twice.
      ...
      if (!tx) {
        ...
        if (!existing)
          return res
            .status(404)
            .json({ error: "Transaction not found for reference." });
      ...
      // Credit what Vibrant actually received (1:1 MXN → MXNe)
      const amountMXNe = Number(data.amount_mxne || data.amount_mxn || tx.amount_mxne);
      ...
      try {
        ...
        // Release the claim so Vibrant's retry can process it again
        tx.status = "pending";
        await tx.save();
        console.error("[Vibrant] MXNe credit failed:", creditErr.message);
        return res.status(502).json({ error: "MXNe credit failed." });
      }

      // 5. Mark transaction as completed
      tx.status = "completed";
      tx.stellar_tx_hash = txHash;
      tx.amount_mxn = Number(data.amount_mxn || tx.amount_mxn);
      tx.amount_mxne = amountMXNe;
      tx.metadata = {
        ...(tx.metadata || {}),
      ...
      await createNotification(
        ...
        "payment",
        "Depósito confirmado",
        `Tu depósito de ${tx.amount_mxn} MXN ha sido acreditado.`,
        tx._id,
      );
      ...
      if (depositor?.email) {
        ...
        await sendEmail(
          depositor.email,
          `Depósito confirmado: ${amountMXNe} MXNe`,
          html,
        );
      }
